...
- Expo 기반 모바일 앱 — 목록에서 권을 고르고, 지면을 넘기며 낭독을 듣는다
- 탭 구성: 읽기 · 더보기(사용법과 수록 안내) · 설정
- 주 500원 구독제 — Entry · Introductory 전체와 Beginner/Essential/Elementary 각 1권은 무료 (결제 연동은 준비 중)
- 구독 검증은 자체 서버 없이 스토어(StoreKit 2 / Play Billing) 조회로만 처리
...
영어 목록 맨 아래에는 인터넷도 시스템 TTS도 없이 동작하는 **오프라인 목소리** `Selton`
(Flite/WASM)이 있습니다. 음질이 낮아 최후의 보루용이며, 앱이 합성을 직접 제어하므로
선택했을 때만 빠르기·음높이 조절이 나타납니다. 한국어는 항상 시스템 TTS로 읽습니다.
