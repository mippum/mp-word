...
├── (tabs)/
│   ├── index.tsx         ← 읽기 (레벨별 책장 — 표지 나열, 접기·펼치기)
...
│                            문장 분리(`splitSentences`)도 여기 있다 (지면이 같이 쓴다)
...
**영영 뜻은 문장 하나씩 읽고, 읽는 문장만 표시한다.** 대본(`utterancesForWord`)이 문장마다
발화를 만들고 `Utterance.part` 에 순번을 넣으면, 플레이어가 그 값을 스냅숏으로 올리고
지면이 그 번호의 문장에만 하이라이트를 준다.

> ⚠️ 대본과 지면이 **같은 `splitSentences()` 로 쪼개야** 한다. 한쪽만 고치면 하이라이트가
> 엉뚱한 문장에 붙는다. 이 함수는 조각을 이어붙이면 원문이 그대로 복원되도록 만들어져 있고,
> 수록된 정의문 3,004개 전부에서 복원을 확인했다.
...
5. **재생 상태와 화면 위치는 따로 움직인다** — 낭독을 따라 쪽을 넘기는 effect 는
   `player.status === 'playing'` 일 때만 돈다. 멈춘 동안에도 따라가게 두면 ◀◀ ▶▶ 로 넘긴
   쪽이 곧바로 되돌아간다 (실제로 겪은 버그). 멈춘 뒤 다른 쪽으로 넘겼으면 재생은
   `resume()` 이 아니라 보이는 쪽부터 시작해야 한다
6. **웹 첫 화면 깜빡임** — `app.json` 의 `web.output: "static"` 때문에 서버 렌더 시점에는
   `localStorage` 가 없어 설정 기본값(모두 펼침 · 시스템 모드)으로 먼저 그려졌다가
   클라이언트에서 저장값으로 교정된다. **웹 개발 모드에서만 보이는 현상**이고 네이티브는 해당 없다
7. **줄바꿈** — 저장소가 LF/CRLF 혼용 경고를 냅니다. CSV 편집 시 전체 파일이 diff로 잡히지 않게 주의하세요
