...
def load_scene_hints() -> tuple[str, dict[str, str]]:
    ...
    expected_keys = {"reasoning_effort", "scene_hints"}
    actual_keys = set(document)
    if actual_keys != expected_keys:
        missing = sorted(expected_keys - actual_keys)
        extra = sorted(actual_keys - expected_keys)
        raise ValueError(
            "scene_hints.json top-level keys are invalid; "
    ...
    for raw_word, raw_hint in payload.items():
        if not isinstance(raw_word, str):
            raise ValueError("every scene_hints.json key must be a string")
        word = raw_word.strip().lower()
        if raw_word != word or not WORD_PATTERN.fullmatch(word):
            raise ValueError(
                f"invalid scene_hints.json word key {raw_word!r}; use lowercase English words"
            )
        filename = filename_for_word(word).lower()
        if filename.removesuffix(".png").upper() in WINDOWS_RESERVED_NAMES:
            raise ValueError(f"{word!r} cannot be used as a Windows filename")
        if filename in filenames and filenames[filename] != word:
            raise ValueError(
                f"words {filenames[filename]!r} and {word!r} map to the same PNG filename"
            )
        if not isinstance(raw_hint, str) or len(raw_hint.strip()) < 30:
...
def main() -> int:
    ...
    if len(sys.argv) != 1:
        print("usage: python gen_img_with_codex.py", file=sys.stderr)
        return 2
    if Image is None:
