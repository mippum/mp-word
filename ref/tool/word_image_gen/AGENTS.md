...
- PNG 생성·후처리·검증이 성공할 때마다 해당 단어 키를 scene_hints.json에서 즉시
  원자적으로 제거합니다. 빈 scene_hints 객체는 모든 이미지가 완료된 정상 상태입니다.
...
- 교육용으로 즉시 이해되는 한 장면
- 또렷한 검은 윤곽선과 자연스러운 비율. 곡선만 반복하지 않고 필요한 곳에 절제된
  직선·각진 윤곽·분명한 모서리를 사용하되 거칠거나 공격적으로 뾰족하게 만들지 않음
- 치비·카와이·버블·유아용 만화·마스코트·장난감풍과 과장된 둥근 머리·짧고 둥근 팔다리 금지
