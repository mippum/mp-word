...
6. 결과를 new/<word>.png에 저장하고 투명도·크기·색상을 검사합니다.
...
- 최소한의 검은 선, 실제 알파 투명 배경, 충분한 여백
- 색상·회색 음영·그라데이션·그림자·텍스처 없음
- 글자·숫자·라벨·로고·워터마크·격자·분할 패널 없음
- 기존 정상 PNG도 새 scene_hints.json을 반영하도록 다시 생성

각 PNG는 Pillow로 무결성, 최소 크기, 알파 채널, 투명 여백, 눈에 띄는 색상을
검사합니다. 한 단어가 최종 실패하면 이전 정상 PNG를 복원하고 다음 단어를 계속
처리합니다.
